...
async function main(): Promise<http.Server> {
  ...
  return httpServer.listen(config.PORT, () => {
    logger.info(`GraphQL Server is now running on ${config.GRAPHQL_ENDPOINT}`);
    if (process.env.NODE_ENV !== 'production') {
      logger.info(`GraphiQL Server is now running on http://localhost:${config.PORT}${config.GRAPHIQL_ROUTE}`);
    }
